...
import { createKerjabakti, updateKerjabakti } from "../../../api/services/kerjabaktiService";
...
export default function KerjabaktiFormComponent({ initialData, isUpdate = false, isDetail = false }: Props) {
...
const handleSubmit = async (e: React.FormEvent) => {
  ...
  if (!formData.hari || !formData.jam_mulai || !formData.jam_selesai || !formData.tempat || !formData.peserta) {
...
const formattedData = {
  ...
  try {
    if (isUpdate && initialData?.id) {
      const response = await updateKerjabakti(initialData.id, formattedData);
      setSuccessMessage(response.message || "Kerja Bakti berhasil diperbarui!");
    } else {
      const response = await createKerjabakti(formattedData);
